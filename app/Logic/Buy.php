...
class Buy
{
    ...
    public function buyStep($request, $buy_info, $address, $user_id)
    {
        ...
        $this->_shop_id = $buy_info['shopInfo']['id'];
        $this->_group_id = $buy_info['groupId'];
    ...
    private function _createOrderStep1()
    {
        ...
        $order_insert['order_price'] = $this->_order_data['TotalPrice'];// 订单总价
        $order_insert['goods_price'] = $this->_order_data['TotalPrice'];// 商品总价
        $order_insert['shop_id'] = $this->_shop_id;// 商铺id
        $order_insert['group_id'] = $this->_group_id;// 团购id
        $order_insert['freight_price'] = 0;// 配送费用
        ...
        // 收货地址
        $order_insert['country'] = $this->_address_data['country_id']??1;// 国家id 1 中国
        ...
        $order_insert['address'] = $this->_address_data['province'] . $this->_address_data['city'] . $this->_address_data['district'] . $this->_address_data['area_name'] . $this->_address_data['address'];// 详细地址
        ...
        if (!$order_model->id) {
      	...
      	$now = Carbon::now();
        foreach ($this->_order_data['cartList'] as $k => $va) {
            if ($va['number']) {
                $order_goods_insert[] = [
                    ...
                    'list_pic_url' => $va['primary_pic_url'],
                    'number' => $va['number'],
                    'goods_specifition_name_value' => $va['goods_specifition_name_value'],
                  	'created_at' => $now,
                    'updated_at' => $now,
                  	'shop_id' => $this->_shop_id,
                ];
                if($order_insert['group_id']==0){
                    $re1 = $this->addSaleNum($va['goods_id'], $va['number'],$va['shop_id'],$va['product_id']);
                    if (!$re1) {
        ...
        // 清除购物车商品
        $cartRe = CartLogic::clearCart($this->_user_id,$this->_shop_id);
    ...
    public function addSaleNum($good_id, $num = 1,$shopId,$product_id)
    {
        $platform_goods = ShopGoods::lockForUpdate()->find($good_id);
        $shop = UserShop::lockForUpdate()->find($shopId);
      	$goods = UserShopProduct::where([
            ['goods_id','=',$good_id],
            ['shop_id','=',$shopId],
            ['product_id','=',$product_id],
        ])->lockForUpdate()->first();
      	$goods_sell = UserShopsShopGoods::where([
            ['good_id','=',$good_id],
            ['shop_id','=',$shopId],
        ])->lockForUpdate()->first();
      
        if (empty($goods) || $goods->stock < $num) {
            return false;
        }
      	
      	$platform_goods->sell_volume = $platform_goods->sell_volume + $num;
      	$platform_goods->save();
        $shop->monthly_sale = $shop->monthly_sale + $num;
        $shop->save();
      	$goods_sell->sell_volume = $goods_sell->sell_volume + $num;
        $goods_sell->save();
    ...
    private function pay_log($msg)
    {
        ...
        file_put_contents('/www/wwwroot/test/storage/log/pay' . date('Y-m-d') . ".log", $msg, FILE_APPEND);
    }
