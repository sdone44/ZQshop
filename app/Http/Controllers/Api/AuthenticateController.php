...
class AuthenticateController extends ApiController
{
    ...
    public function auto_login(Request $request)
    {
        ...
        if ($userInfo && $userInfo->toArray()) {
            //执行登录
          	$newUserInfo = \GuzzleHttp\json_decode($request->input('rawData'),true);
          
            $userInfo->login_ip = $this->getClientIP();
            $userInfo->login_time = Carbon::now();
          
          	$userInfo->avatar = $newUserInfo['avatarUrl'];
            $userInfo->nickname = $newUserInfo['nickName'];
          	
            $userInfo->save();
            // 直接创建token
            $token = $userInfo->createToken($openid)->accessToken;
            $resData = compact('token','userInfo');
            $resData['defaultAddress'] = ShopAddress::getDefault($userInfo->id);
