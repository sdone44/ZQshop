...
use App\Admin\Extensions\TurnoverStatisticsTools;
...
class TurnoverStatisticsController  extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(UserShop::class, function (Grid $grid) {
			...
			$grid->model()->whereIn('shop_type', [1,2]);//1-平台商家 2-自营商家
            ...
            $grid->column('更多信息')->expand(function () use($getListImg) {
                ...
                $drawRecord = DrawRecord::getDrawRecordArrByShopId($this->id);
                $drawingRecordRes = new Table(['提现记录'],$drawRecord);
                $tab->add('提现记录', $drawingRecordRes);
                return $tab;
            }, '更多信息');

            $grid->actions(function ($actions) {
                $actions->prepend(new TurnoverStatisticsTools($actions->getKey()));
    ...
    public function draw(Request $request)
    {
        ...
        $userShopModel = UserShop::where('id',$id)->first();
        $totalVal = ShopOrder::getShopTotalDateById($id)['total_retail_value'];
        $remainingVal = $totalVal - $userShopModel['withdrawals'];

        if($val > $remainingVal){
            return 3;
        }

        $re = UserShop::where('id',$id)->increment('withdrawals',$val);

        if($re){
            //写入记录
            $DrawRecord = new DrawRecord;
            $DrawRecord->shop_id = $id;
            $DrawRecord->withdrawals = $val;
            $DrawRecord->save();
            return 1;
        }else{
            return 0;
