...
class ShopGroupOrderController extends Controller
{
    use ModelForm;
                    ...
                    setInterval(function(){
                        $.ajax({
                            ...
                            success : function(re) {
                                if(re){
                                    console.log("新订单来啦~");
                                    $(".box-header.with-border").html("<audio id=\'newOrderNotify\' autoplay=\'autoplay\'><source src=\'/newOrderNotify.mp3\' type=\'audio/mp3\'/></audio>");
                                    $("#newOrderNotify").remove();
                                    swal({ 
                                        title: "您有新的订单！", 
                                        type: "warning", 
            ...
            if(!Admin::user()->isAdministrator()){
                $grid->actions(function ($actions) {
                   $actions->disableDelete();
                    ...
                    $actions->disableView(); 
                    // prepend一个操作
                    $actions->prepend(new OrderTools($actions->getKey()));
                });
            }
