        ...
        return Admin::content(function (Content $content) {
            ...
            $content->row(function (Row $row) {
                ...
                $row->column(6, function (Column $column) {
                    ...
                    $form->select('parent_id','上级分类')->options(ShopCategory::selectOptions())->help('Root为顶级分类');
                    $form->text('name','分类名称')->rules('required|min:2')->help('必填');
                    //$form->text('front_name','分类简述')->rules('required|min:2')->help('必填');
                    $form->number('sort_order', '排序')->default(255)->help('数值越大越往后排列');
